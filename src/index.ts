...
export async function recognize(
    ...
    for (let i = 0; i < boxes.length; i++) {
        ...
        for (let i = 0; i < width_num; i++) {
            ...
            const text = recResult.outputResult(ocr_character);
            text_list_tmp = text_list_tmp.concat(text.text);
            console.log("텍스트 = " , text_list_tmp)
        }
        text_list.push(text_list_tmp);
        console.log("결과" ,text_list)
    }
    return { text: text_list, points: point };
}

async function loadCharacterData() {
    const response = await fetch('/ppocr_keys_v1.txt');
    if (!response.ok) {
        throw new Error('Failed to load character data');
    }
    const text = await response.text();
    ocr_character = text.split('\n'); // 각 줄을 분리하여 배열로 저장
}

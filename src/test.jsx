...
import {VideoRecorder, VideoUtil} from "./video.js";

const ScreenView = () => {
  const [tab, setTab] = useState(0);
  const [stream, setStream] = useState(null);

  const [videoOptions, setVideoOptions] = useState(true);
  ...
  async function getDisplayMedia() {
    ...
    return await navigator.mediaDevices.getDisplayMedia(constraints).then(stream => {
      ...
      console.log("음",text.toString())
    });
  }

  const controlVideo = function (type) {
